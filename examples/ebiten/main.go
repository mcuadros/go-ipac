	...
	for id := range g.gamepadIDs {
		...
		for b := ebiten.GamepadButton(0); b < maxButton; b++ {
			...
			if inpututil.IsGamepadButtonJustPressed(id, b) {
				g.ipac.SetLEDColor(int(b), color.Black)
				log.Printf("button pressed: id: %d, button: %d", id, b)
			}
			if inpututil.IsGamepadButtonJustReleased(id, b) {
				g.ipac.SetLEDColor(int(b), color.White)
...
func (g *Game) handleInput(button int) {

	g.ipac.SetLEDColor(0, color.RGBA{255, 0, 0, 0})
	g.ipac.SetLEDColor(1, color.RGBA{0, 255, 0, 0})
	g.ipac.SetLEDColor(2, color.RGBA{0, 0, 255, 0})
	g.ipac.SetLEDColor(3, color.RGBA{255, 255, 0, 0})
...
func main() {
	ipac := ipac.NewIPacUltimateIO()
	defer ipac.Close()

	err := ipac.InitDevices()
	if err != nil {
		fmt.Printf("Failed to initialize HID devices: %v\n", err)
		return
	}

	ipac.SetLEDFadeTime(0)
	ipac.SetLEDIntensity(-1, 255)

	ebiten.SetWindowSize(screenWidth, screenHeight)
	ebiten.SetWindowTitle("Gamepad (Ebitengine Demo)")
	if err := ebiten.RunGame(&Game{ipac: ipac}); err != nil {
		log.Fatal(err)
	}
}
